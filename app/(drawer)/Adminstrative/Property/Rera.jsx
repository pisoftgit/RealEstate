import React, { useState } from 'react';
import { View, Text, TextInput, TouchableOpacity, StyleSheet, SafeAreaView, ScrollView, ActivityIndicator, Alert } from 'react-native';
import { Ionicons, Feather } from '@expo/vector-icons';
import { useNavigation } from 'expo-router';
import useReraActions from '../../../../hooks/useReraActions'; // ✅ updated hook
import { widthPercentageToDP as wp, heightPercentageToDP as hp } from 'react-native-responsive-screen';

export default function Rear() {
  const navigation = useNavigation();
  const [rear, setRear] = useState('');
  const [editId, setEditId] = useState(null);

  const { reras, loading, addRera, updateRera, deleteRera } = useReraActions();

  // ✅ Handle Add / Update
  const handleSubmit = async () => {
    if (!rear.trim()) {
      Alert.alert('Validation', 'Please enter RERA');
      return;
    }

    try {
      if (editId) {
        await updateRera({ id: editId, name: rear }); // 🔹 fixed payload key
        Alert.alert('Success', 'RERA updated successfully!');
      } else {
        await addRera({ name: rear }); // 🔹 fixed payload key
        Alert.alert('Success', 'RERA added successfully!');
      }
      setRear('');
      setEditId(null);
    } catch (error) {
      Alert.alert('Error', 'Something went wrong. Try again!');
      console.log(error);
    }
  };

  // ✅ Handle Delete
  const handleDelete = (id) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this RERA?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: async () => {
          try {
            await deleteRera(id);
            Alert.alert('Deleted', 'RERA deleted successfully!');
          } catch (error) {
            Alert.alert('Error', 'Failed to delete RERA.');
          }
        },
      },
    ]);
  };

  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        <View style={styles.container}>
          {/* Header */}
          <View style={styles.header}>
            <TouchableOpacity onPress={() => navigation.openDrawer()}>
              <Ionicons name="menu" size={28} color="black" />
            </TouchableOpacity>
            <Text style={styles.title}>RERA</Text>
          </View>

          {/* Form Card */}
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{editId ? 'Update RERA' : 'Configure RERA'}</Text>
            <View style={styles.formRow}>
              <Text style={styles.label}>RERA</Text>
              <TextInput
                style={styles.input}
                value={rear}
                onChangeText={setRear}
                placeholder="Enter RERA Number"
              />
            </View>
            <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
              <Text style={styles.submitButtonText}>{editId ? 'Update' : 'Submit'}</Text>
            </TouchableOpacity>
          </View>

          {/* Table Card */}
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Existing RERA List</Text>
            {loading ? (
              <ActivityIndicator size="large" color="#5aaf57" style={{ marginTop: 16 }} />
            ) : (
              <>
                <View style={styles.tableHeader}>
                  <Text style={[styles.tableHeaderText, { flex: 0.7 }]}>S. No</Text>
                  <Text style={[styles.tableHeaderText, { flex: 2 }]}>RERA</Text>
                  <Text style={[styles.tableHeaderText, { flex: 1 }]}>Action</Text>
                </View>

                {reras && reras.length > 0 ? (
                  reras.map((item, idx) => (
                    <View key={item.id} style={styles.tableRow}>
                      <Text style={[styles.tableCell, { flex: 0.7 }]}>{idx + 1}</Text>
                      <Text style={[styles.tableCell, { flex: 2 }]}>{item.name}</Text>
                      <View style={[styles.actionCell, { flex: 1 }]}>
                        <TouchableOpacity
                          style={styles.iconBtn}
                          onPress={() => {
                            setRear(item.name);
                            setEditId(item.id);
                          }}
                        >
                          <Feather name="edit" size={18} color="#5aaf57" />
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={styles.iconBtn}
                          onPress={() => handleDelete(item.id)}
                        >
                          <Ionicons name="trash" size={18} color="#d32f2f" />
                        </TouchableOpacity>
                      </View>
                    </View>
                  ))
                ) : (
                  <Text style={{ textAlign: 'center', marginTop: 16, color: '#666' }}>
                    No RERA found
                  </Text>
                )}
              </>
            )}
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
}

// ✅ Styles
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#f8f9fa' },
  container: { flex: 1, backgroundColor: '#f8f9fa', padding: wp('5%') },
  header: { paddingVertical: hp('2.2%'), marginBottom: hp('1%') },
  title: { fontSize: wp('8%'), fontFamily: 'PlusSB', color: '#333', marginLeft: wp('4%') },
  card: { backgroundColor: '#fff', borderRadius: wp('4%'), padding: wp('4%'), marginBottom: hp('2%'), elevation: 3 },
  cardTitle: { fontSize: wp('4.5%'), fontFamily: 'PlusSB', color: '#333', marginBottom: hp('1.5%') },
  formRow: { flexDirection: 'row', alignItems: 'center', marginBottom: hp('1.5%') },
  label: { width: wp('20%'), color: '#333', fontFamily: 'PlusR', fontSize: wp('3.5%') },
  input: { flex: 1, borderWidth: 1, borderColor: '#e0e0e0', borderRadius: wp('2%'), padding: wp('2%'), backgroundColor: '#f5f5f5', color: '#333', fontFamily: 'PlusR', fontSize: wp('3.5%') },
  submitButton: { backgroundColor: '#5aaf57', paddingVertical: hp('1.2%'), borderRadius: wp('2%'), alignItems: 'center', marginTop: hp('1%') },
  submitButtonText: { color: '#fff', fontSize: wp('4%'), fontFamily: 'PlusSB' },
  tableHeader: { flexDirection: 'row', backgroundColor: '#5aaf57', padding: wp('2%'), borderRadius: wp('2%'), marginBottom: hp('0.5%') },
  tableHeaderText: { flex: 1, color: '#fff', textAlign: 'center', fontFamily: 'PlusSB', fontSize: wp('3.2%') },
  tableRow: { flexDirection: 'row', padding: wp('2%'), borderBottomWidth: 1, borderBottomColor: '#f0f0f0', backgroundColor: '#fff' },
  tableCell: { flex: 1, textAlign: 'center', color: '#333', fontFamily: 'PlusR', fontSize: wp('3%') },
  actionCell: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: wp('3%') },
  iconBtn: { padding: wp('1%') },
});
